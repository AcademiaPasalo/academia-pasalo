...
describe('E2E: Integridad de Seguridad y Blindaje de Accesos', () => {
  ...
  describe('BLINDAJE PUNTO 4: Consistencia de Visibilidad Dinámica', () => {
    it('debe filtrar materiales futuros incluso si ya existen en la caché completa', async () => {
      ...
      await dataSource.query('UPDATE material_folder SET visible_from = ? WHERE id = ?', [yesterday, pc1FolderId]);
      await cacheService.del(`cache:material-folders:roots:${evaluationPC1.id}`);
      await cacheService.del(`cache:material-folders:contents:${pc1FolderId}`);
      ...
      // Pero internamente el sistema ya llenó la caché con la lista completa de la BD
      ...
      expect(mats1.find((m: any) => m.id === futureMatId)).toBeUndefined();

      // 3. Hack técnico: Movemos el 'visibleFrom' a AYER en la BD
      // Usamos el ID como número para asegurar compatibilidad con la BD
      await dataSource.query('UPDATE material SET visible_from = ? WHERE id = ?', [yesterday, futureMatId]);

      // 4. Alumno consulta de nuevo -> Debe ver el material AHORA
      // NOTA: El sistema NO debe haber invalidado la caché de esta carpeta por este UPDATE manual.
      // El objetivo es demostrar que aunque la caché tenga el registro viejo, el filtro 'applyVisibilityFilter'
      // lo evalúa contra el 'now' actual de la petición.
      ...
      const found = mats2.find((m: any) => String(m.id) === String(futureMatId));
