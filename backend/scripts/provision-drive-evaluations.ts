...
async function main(): Promise<void> {
  ...
  if (!targetEmail) {
    throw new Error(
      'Uso: npx ts-node -r tsconfig-paths/register scripts/provision-drive-evaluations.ts <email>',
    );
  }
  ...
  try {
    ...
    for (const row of evaluations) {
      ...
      try {
        ...
        if (!provisioned.viewerGroupEmail) {
          throw new Error(
            `No se obtuvo viewerGroupEmail para evaluationId=${evaluationId}`,
          );
        }

        await workspaceGroupsService.ensureMemberInGroup({
          groupEmail: provisioned.viewerGroupEmail,
    ...
    console.log('----------------------------------------');
    console.log(`Provision completada. OK=${ok} FAIL=${failed}`);
    if (failures.length) {
      console.log('Errores:');
