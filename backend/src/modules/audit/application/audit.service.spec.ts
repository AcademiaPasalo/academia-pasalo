...
describe('AuditService', () => {
  ...
  const mockAuditAction = { id: '1', code: 'TEST_ACTION', name: 'Test Action' } as AuditAction;

  beforeEach(async () => {
    auditLogRepository = {
      create: jest.fn().mockImplementation((dto) => Promise.resolve({ id: 'log-1', ...dto })),
  ...
  describe('logAction', () => {
    it('should create an audit log successfully', async () => {
      const result = await service.logAction('user-1', 'TEST_ACTION', 'entity', '123');
      
      expect(auditActionRepository.findByCode).toHaveBeenCalledWith('TEST_ACTION', undefined);
    ...
    it('should throw InternalServerErrorException if action code does not exist', async () => {
      ...
      await expect(service.logAction('user-1', 'INVALID_ACTION'))
        .rejects
  ...
  describe('getUnifiedHistory', () => {
    it('should merge and sort events from both sources', async () => {
      ...
      (securityEventRepository.findAll as jest.Mock).mockResolvedValue(mockSecurityEvents);
      (auditLogRepository.findAll as jest.Mock).mockResolvedValue(mockAuditLogs);
